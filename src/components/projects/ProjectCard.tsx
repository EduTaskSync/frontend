...
export const ProjectCard = ({ project }: ProjectCardProps) => {
  ...
  const { deleteProjectResponse } = useProjects();
  // Use useRef instead of useState to track the timeout
  const deleteTimeoutRef = useRef<NodeJS.Timeout | null>(null);

  const handleDeleteClick = (e: React.MouseEvent) => {
    e.preventDefault(); // Prevent navigation to groject page
  ...
  const confirmDelete = () => {
    ...
    toast.info(`Deleting ${project.name}`, {
      description: (
        <div>
      ...
      action: {
        ...
        onClick: () => {
          ...
          if (deleteTimeoutRef.current) {
            clearTimeout(deleteTimeoutRef.current);
            deleteTimeoutRef.current = null;
            toast.success(`Deletion of ${project.name} canceled`);
          }
        },
      },
    });
  ...
  return (
    <div className="w-full p-[2px] rounded-xl bg-gradient-to-br from-purple-400 via-pink-300 to-indigo-400 shadow-[0_2px_10px_0px_rgba(0,0,0,0.1)] transition-all duration-300 hover:shadow-[0_0px_20px_5px_rgba(168,85,247,0.25)] hover:from-purple-500 hover:via-pink-400 hover:to-indigo-500 groject/wrapper">
      <Link
        to={`${project.id}`}
        state={{
          projectDetails: {
        ...
        className="w-full groject/card block h-full relative"
      >
        <div className="absolute top-2 right-2 z-20 opacity-0 groject-hover/card:opacity-100 transition-opacity duration-200">
          <Button
            variant="destructive"
            size="sm"
        ...
        >
          <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-black/20 backdrop-blur-[2px] opacity-60 transition-opacity duration-300 groject-hover/card:opacity-75"></div>
          {/* Content container */}
          <div className="flex flex-col justify-end h-full z-10">
            <div className="inline-flex flex-col px-3 py-2 bg-black/30 border border-white/10 backdrop-blur-md rounded-lg transition-all duration-300 groject-hover/card:bg-black/40 max-w-full">
              ...
              <div className="flex items-center">
                ...
                >
                  ...
                  <span className="font-sans font-bold">{project.size}</span>
                </Badge>
              </div>
            </div>
          </div>
        ...
        <AlertDialogContent className="max-w-md shadow-lg">
          <AlertDialogHeader>
            <AlertDialogTitle className="font-heading text-xl">
              <span className="text-purple-300">Delete</span> Groject
            </AlertDialogTitle>
