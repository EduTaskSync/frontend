...
export const KanbanColumn = React.memo(
  ({ column, onDeleteCol, isDeletePending, isEditPending, onEditCol }: KanbanColumnProps) => {
    ...
    const { data, isLoading, isError, error } = getKanbanTasksResponse;
    ...
    // Determine gradient based on column accent color
    const gradientStyle = isDragging
      ? `bg-gradient-to-br ${
          accent === 'blue'
    ...
    return (
      <div
        ...
        className={cn(
          ...
          isDragging ? gradientStyle : 'bg-background',
          `bg-gradient-to-br ${borderGradient} shadow-[0_2px_10px_0px_rgba(0,0,0,0.1)] hover:shadow-[0_0px_15px_5px_rgba(56,189,248,0.15)]`
        )}
      >
        <Card className="flex flex-col h-[800px] w-full rounded-[calc(0.75rem-1px)] border-0 shadow-sm overflow-hidden bg-card/95 backdrop-blur-sm ">
          ...
          <CardContent className="flex-1 p-2 space-y-3 overflow-hidden">
            {/* Task List */}
            <div className="h-full">
              {isLoading ? (
                ...
                <div
                  className={cn(
                    'rounded-lg border border-dashed p-4 flex flex-col items-center justify-center h-32',
